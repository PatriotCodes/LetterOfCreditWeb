...
export class StartupCheckComponent implements OnInit {
  ...
  constructor(private http: Http, private router: Router, public portProvider: PortProviderService, public urlProvider: UrlProviderService) {}
  ...
  scanForNodes() {
    ...
    for (let i of this.portProvider.nodes) {
      let url = this.urlProvider.url + i + '/webserver/loc/api/loc/me';
      this.http.get(url)
        .toPromise()
